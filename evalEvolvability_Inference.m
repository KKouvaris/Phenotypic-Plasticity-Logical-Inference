function [ fitDiff, initFit_Th ] = evalEvolvability_Inference(initPop, scenario)
...
%normalise
for i = 1:numDirs
    directions(i,:) = directions(i,:)/norm(directions,2);
end
    ...
    for t=1:t_max
        ...
        for i=1:N
            ...
            while(~childFound)
                ...
                %fitness
                d = sum((P-2^-.5).*directions(theta,:)');
                m = sum(abs(directions(theta,:)));
                newPop(i).fitness = (1+s)^(d/m);
                childFound = 1;
            end
        end
