function [xvals, yvals, color] = hintmat(w, max_element)
...
% Set scale to be up to 0.98 of maximum absolute weight value, where scale
% defined so that area of box proportional to weight value.

w = flipud(w);
[nrows, ncols] = size(w);

%scale = 0.49*sqrt(abs(w)/max(max(abs(w))));
scale = 0.49*sqrt(abs(w)/max_element);
scale = scale(:);
color = 0.5*(sign(w(:)) + 3);
