function [ fitDiff, initFit_Th ] = evalEvolvability( initPop, scenario)
...
for theta = theta_values
    ...
    if (scenario == 2), env_cue = [sind(theta) cosd(theta)]; else  env_cue = zeros(1,epsilon);  end
    %if (scenario == 2), env_cue = rand(1,epsilon)-0.5; else  env_cue = zeros(1,epsilon);  end
    ...
    initFit_Th(1,theta==theta_values) = initFit;
